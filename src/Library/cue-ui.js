...
/**Cue UI Plugin
 * API for top-down, procedural DOM Manipulation.
 * When a component responds to changes of state, it follows predefined procedures to format itself
 * to visually reflect the new state. It does so strictly by altering its attributes and child contents. (top down)
 * These procedures are called reactions in Cue.
 * Reactivity is the automatic triggering of those reactions whenever the corresponding state changes.
 * Reactions are procedural manipulations of attributes and children of the component. They describe the steps taken
 * to format a piece of raw state data into the component-specific visual representation of it.
 * Cue UI Plugin aims to provide a semantic API to describe common visual formatting operations.
 * */
  ...
  return Object.assign(Library.ui, {
    ...
    select(x, within) {
      ...
      if (typeof x === 'string') {
        ...
        const parent = within ? this.select(within) : doc;
        let node;
    ...
    fromHTML(str) {
      ...
      if (typeof str !== 'string') throw new TypeError(`Can't create DOM element(s) from non-string of type ${typeof str}.`);
      str = str.trim();
      const dom = doc.createRange().createContextualFragment(str);
    ...
    getRefs(parentNode) {
      ...
      parentNode = this.select(parentNode);
